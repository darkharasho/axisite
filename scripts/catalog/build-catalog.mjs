...
if (import.meta.url === `file://${process.argv[1]}`) {
  const overrides = JSON.parse(readFileSync('public/addon-checker/data/overrides.json', 'utf8')).overrides ?? {}
  const assessments = existsSync('public/addon-checker/data/conduct.json')
    ? JSON.parse(readFileSync('public/addon-checker/data/conduct.json', 'utf8')) : {}
  const previous = existsSync('public/addon-checker/data/catalog.json')
    ? JSON.parse(readFileSync('public/addon-checker/data/catalog.json', 'utf8')) : null
  ...
  writeFileSync('public/addon-checker/data/catalog.json', JSON.stringify(catalog, null, 2) + '\n')
  const { discovered, reused, enriched, deferred, failures } = catalog.stats
  console.log(
    `wrote ${catalog.repos.length} repos to public/addon-checker/data/catalog.json ` +
