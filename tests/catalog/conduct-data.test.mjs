...
import { CONDUCT_TIERS, ADVANTAGE_TIERS, indexAssessments, isContentious } from '../../public/addon-checker/conduct.mjs'

const doc = JSON.parse(readFileSync('public/addon-checker/data/conduct.json', 'utf8'))
const assessments = Object.entries(doc.assessments)
const catalog = JSON.parse(readFileSync('public/addon-checker/data/catalog.json', 'utf8'))
const policyIds = new Set(JSON.parse(readFileSync('public/addon-checker/data/policies.json', 'utf8')).clauses.map((c) => c.id))

const repos = new Map(catalog.repos.map((r) => [r.full_name.toLowerCase(), r]))

describe('public/addon-checker/data/conduct.json', () => {
